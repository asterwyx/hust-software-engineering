...
typedef struct map_node {
    int id;
    int type;                   // 地图结点类型，枚举类型为 NodeType
    int estate_lvl;             // 房产等级，type==VACANCY 时有效。枚举类型为 EstateLevel
    player_t* owner;            // 房产的拥有者，type==VACANCY 时有效。枚举类型为 PlayerIdx
    vector<player_t*> players;  // 当前处于该结点的玩家。枚举类型为 PlayerIdx
    int item;                   // 当前置于该结点的道具。枚举类型为 ItemType 或 None
    int value;                  // type==VACANCY 时为空地价格，计算房产价值时乘以（房产等级+1）；type==MINE 时为可获取的点数
...
typedef vector<map_node_t> map_t;
