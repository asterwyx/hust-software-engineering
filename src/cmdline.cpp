...
void do_robot(player_t* player)
{
    apply_item(*get_map(), *player, ROBOT);
}


void do_block(int step, player_t* player)
{
    apply_item(*get_map(), *player, BLOCK, step);
}


void erase_player_from_curr_pos(player_t* p_player)
{
    map_t* p_map = get_map();
...
void switch_player(player_t** p_next_player)
{
    auto players = get_player_vec();
